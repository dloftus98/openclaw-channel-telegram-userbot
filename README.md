...
If you want to test local changes before publishing:

```bash
git clone https://github.com/overpod/openclaw-channel-telegram-userbot.git
cd openclaw-channel-telegram-userbot
bun install
npm pack
openclaw plugins install ./openclaw-channel-telegram-userbot-0.3.0.tgz
```

Tip: if your local `package.json` version changes, replace the tarball filename with the version generated by `npm pack`.

To avoid installing a stale tarball, prefer:
...
you are almost certainly installing an older `.tgz` from before these keys were present. Delete old tarballs and re-run `npm pack`.
